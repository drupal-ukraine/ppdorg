...
mysql_user { 'drupal_read_only@localhost':
	ensure        => 'present',
	password_hash => mysql_password('drupal_read_only'),
	provider      => 'mysql',
}

mysql_grant { "drupal_read_only@localhost/drupal.*":
	ensure     => present,
	user       => "drupal_read_only@localhost",
	table      => 'drupal.*',
	privileges => [ 'SELECT' ],
}

# Remote mysql read/write user for DIS.
mysql_user { 'drupal@%':
	ensure        => 'present',
...
# Documentation about gearman class https://forge.puppetlabs.com/saz/gearman
# class { 'gearman':
# maxfiles => 16384,
# }
#$ensure = 'present'
